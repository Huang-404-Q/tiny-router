...
Defines the local Anthropic-compatible gateway contract for assistant-directed next-turn model routing. The gateway keeps provider details private in local configuration while allowing the assistant to choose a configured route category such as `control` or `execution` for the next request.
